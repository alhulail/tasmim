...
export function UserMenu({ user, profile, locale }: UserMenuProps) {
  ...
  const getPlanLabel = (plan: string | null | undefined) => {
    const labels: Record<string, { en: string; ar: string }> = {
      ...
      one_time: { en: 'Package', ar: 'الحزمة' },
    };
    return labels[plan || 'free']?.[isArabic ? 'ar' : 'en'] || plan;
  };
      ...
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="relative h-10 w-10 rounded-full">
          <Avatar className="h-10 w-10 border-2 border-stone-200">
            ...
            <AvatarFallback className="bg-gradient-to-br from-indigo-100 to-violet-100 text-indigo-600 font-medium">
      ...
      <DropdownMenuContent className="w-56" align="end" forceMount>
        <DropdownMenuLabel className="font-normal">
          <div className="flex flex-col space-y-1">
            ...
            <span className="mt-1 inline-flex items-center self-start rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-medium text-indigo-700">
        ...
        >
          <CogIcon className="me-2 h-4 w-4" />
